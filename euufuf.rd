...
import { PhoneOutgoing, Phone, PhoneOff, FileText, Loader, Volume2, VolumeX } from "lucide-react"
...
const VoiceCallComponent = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && "speechSynthesis" in window) {
      ...
      console.log("Text-to-Speech مدعوم في هذا المتصفح")
    } else {
      console.log("Text-to-Speech غير مدعوم في هذا المتصفح")
    }
  }, [])
  ...
  const speakText = useCallback(
    (text) => {
      if (!speechSupported || !text.trim()) {
        console.log("النص فارغ أو الميزة غير مدعومة")
        return
      }
      ...
      // البحث عن صوت ذكوري عربي أولاً
      let selectedVoice = voices.find(
        (voice) =>
          (voice.lang.startsWith("ar") || voice.name.includes("Arabic") || voice.name.includes("عربي")) &&
          (voice.name.toLowerCase().includes("male") ||
            voice.name.toLowerCase().includes("man") ||
            voice.name.toLowerCase().includes("masculine") ||
            voice.name.toLowerCase().includes("ذكر") ||
            voice.name.toLowerCase().includes("رجل")),
      )

      // إذا لم نجد صوت ذكوري عربي، ابحث عن أي صوت ذكوري
      if (!selectedVoice) {
      ...
      // إذا لم نجد صوت ذكوري، ابحث عن أي صوت عربي (قد يكون أنثوي)
      if (!selectedVoice) {
        selectedVoice = voices.find(
          (voice) => voice.lang.startsWith("ar") || voice.name.includes("Arabic") || voice.name.includes("عربي"),
        )
      }

      // إذا لم نجد أي صوت عربي، استخدم أول صوت ذكوري متاح
      if (!selectedVoice) {
        selectedVoice = voices.find(
          (voice) =>
            voice.name.toLowerCase().includes("male") ||
      ...
      if (selectedVoice) {
        ...
        console.log(
          ...
          selectedVoice.name,
          "- النوع:",
          selectedVoice.name.toLowerCase().includes("male") || selectedVoice.name.toLowerCase().includes("man")
            ? "ذكر"
            : "غير محدد",
        )
      } else {
        console.log("لم يتم العثور على صوت مناسب، استخدام الصوت الافتراضي")
      }
  ...
  // وظيفة نسخ النص المحول مع تحديث لتشمل إيقاف القراءة
  const copyTranscript = useCallback(() => {
    if (transcriptText) {
      navigator.clipboard.writeText(transcriptText)
      // إيقاف القراءة عند النسخ
      if (isSpeaking) {
        stopSpeaking()
      }
    }
  }, [transcriptText, isSpeaking, stopSpeaking])
          ...
          <div className="text-center space-y-4">
            ...
            {isSpeaking && (
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-green-300 text-sm">جاري قراءة النص...</span>
              </div>
            )}
                ...
                {!isRecording && audioURL && !isTranscribing && (
                  <div className="flex items-center space-x-2">
                    <div className="px-3 py-1 bg-gradient-to-r from-green-500/20 to-blue-400/20 backdrop-blur-sm rounded-full border border-white/10">
                      <span className="text-white/80 text-xs">{isPlaying ? "جاري التشغيل..." : "اضغط للتشغيل"}</span>
                    </div>
                    <Button
            ...
            <div className="w-full max-w-sm bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {speechSupported && (
                    <Button
                      onClick={isSpeaking ? stopSpeaking : () => speakText(transcriptText)}
                      className={`px-2 py-1 text-white text-xs rounded-lg transition-all duration-200 ${
                        isSpeaking ? "bg-red-500/90 hover:bg-red-500" : "bg-blue-500/90 hover:bg-blue-500"
                      }`}
